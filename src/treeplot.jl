...
function coordinates_horizontal(tree::Root)
    ...
    states = Dict{Int64, Vector{Int64}}()
...
function find_y!(node::TipNode, y::Vector{Float64})
...
function find_vertical!(node::TipNode, x, y, states, t)
    return(node.index)
end
...
function find_x!(::TipNode, ::Dict{Int64}, states, t::Float64)
end
...
function node_depths_po!(node::TipNode, depths::Vector{Float64}, t::Float64)
    depths[node.index] = t
end
